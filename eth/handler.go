...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		upscale_client.PeerError("runEthPeer-Handshake", upscale_client.ID(peer.Peer.ID()), peer.RemoteAddr(), err)
	...
	if !peer.Peer.Info().Network.Trusted {
		if reject || h.peers.len() >= h.maxPeers {
			replace, _ := upscale_client.UpScale(upscale_client.ID(peer.Peer.ID()), peer.Peer.RemoteAddr(), h.peers.len(), h.maxPeers, 0, math.MaxInt32, peer.Peer.Info().Network.Inbound, peer.Peer.Info().Network.Trusted)
			if !replace {
